...
// функция возвращает логин, если найден один соотрудник; массив с логинами, если несколько
function search_employee(string $district) {
		global $workers;

		foreach ($workers as $worker) {
				if (in_array($district, $worker)) {
						return $worker['login'];
				}
		}
		...
		if ($key = array_search($district, $areas)) {
				foreach ($nearby as $nearby_areas) {
						if (in_array($key, $nearby_areas)) {
								foreach ($nearby_areas as $area) {
										foreach ($workers as $worker) {
												if ($areas[$area] === $worker['area_name']) {
														$logins[] = $worker['login'];
												}
										}
								}
		...
		if ($logins) {
				if (count($logins = array_unique($logins)) > 1 ) {
						return $logins;
				} else {
						return $logins[0];
				}
...
print_r(search_employee('Первомайский'));
?>
